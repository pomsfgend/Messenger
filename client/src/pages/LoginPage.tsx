...
const LoginPage: React.FC = () => {
    ...
    const [twoFactorInfo, setTwoFactorInfo] = useState<{ userId: string } | null>(null);
    ...
    const handleLoginResponse = (response: any) => {
        if (response.twoFactorRequired) {
            setTwoFactorInfo({ userId: response.userId });
        } else {
    ...
    const handlePhoneLogin = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = await phoneLogin(phoneNumber, code);
            handleLoginResponse(response);
        } catch (error: any) {
             handleAuthError(error);
        } finally {
    ...
    }, [googleLogin, handleAuthError, handleLoginResponse]);

    const handleGoogleError = useCallback(() => {
        toast.error(t('toast.googleLoginError'));
    }, [t]);
    
    const handleTelegramSuccess = useCallback(async (user: any) => {
        setLoading(true);
        try {
    ...
    }, [telegramLogin, handleAuthError, handleLoginResponse]);
    ...
    useEffect(() => {
        ...
        if (telegramContainerRef.current) {
            ...
            if (!telegramBotUsername) {
                console.warn("Telegram Bot Username is not set in client/src/config.ts.");
                return;
            }
            ...
            <div className="max-w-md w-full p-4 sm:p-8 space-y-4 animate-fade-in soft-panel mt-24 mb-8">
                <div className="bg-slate-200/30 dark:bg-slate-800/30 p-4 sm:p-6 rounded-xl space-y-4">
                    <div className="text-center space-y-2 mb-4">
                        ...
                        <p className="text-slate-500 dark:text-slate-400">{t('login.signInPrompt')}</p>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <div ref={telegramContainerRef} id="telegram-login-container" className="min-h-[52px] w-full flex items-center justify-center"></div>
                    </div>

                    <form onSubmit={handleAnonymousLogin} className="space-y-2">
                        <div className="soft-panel-inset p-1">
